...
def load_model(args, device):
    ...
    if n_frames_ckpt > 1:
        model = RMOS4DUNetV2(1, args.num_classes, base_channels=args.base_channels).to(device)
        print(f"[INFO] Loaded 4D model (n_frames={n_frames_ckpt})")
    else:
        model = RMOSUNet(1, args.num_classes, D=3, base_channels=args.base_channels).to(device)
